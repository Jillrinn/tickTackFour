...
- [ ] 5.2 40件超過時の自動削除機能
  - 全エンティティ取得とRowKey降順ソート
  - 40件を超える古いエンティティの特定
  - 古いエンティティの削除処理
  - _要件: 1.4_

- [ ] 5.3 エラーハンドリングの実装
  - バリデーションエラー時の400エラー返却
...
- [ ] 6. usePlayerNameHistory カスタムフックの実装
- [ ] 6.1 基本的な取得機能の実装
...
- [ ] 9.2 GET /api/player-names のテスト
  - 40件取得成功テスト
  - 10件のみ存在時の10件返却テスト
  - Cosmos DB読み取り失敗時の空配列返却テスト
  - _要件: 2.1, 2.2, 8.6_

- [ ] 9.3 POST /api/player-names のテスト
  - 有効な名前配列の保存成功テスト
...
- [ ] 10. フロントエンドユニットテストの作成
- [ ] 10.1 usePlayerNameHistory フックのテスト
