...
## タスク分割の必須ルール（UI/バックエンド分離）

**重要**: フロントエンド機能を実装する際は、以下のルールを**必ず**遵守すること：

### 1. UIとバックエンドの明確な分離

**全ての機能タスクは、以下のように分割する**:
...
#### ✅ 良い例（UI/バックエンド分離）:
```markdown
- [ ] 4.1 ターン時間トラッキング機能（バックエンド/ロジック層）
  - **ファイル**: frontend/src/hooks/useGameState.ts
  - getCurrentTurnTime関数を実装（秒単位で返す）
  - turnStartedAtフィールドを使用して計算
  - 一時停止中の時間を除外するロジック
  - **完了条件**: ユニットテストパス

- [ ] 4.2 ターン時間表示UI（フォールバックモード）
  - **ファイル**: frontend/src/components/GameTimer.tsx（lines 327-372）
  - data-testid="turn-time"属性を追加
  - アクティブプレイヤーのみ表示
  - 1秒ごとに再レンダリング（forceUpdate）
  - **完了条件**: ブラウザで手動確認 + ユニットテスト

- [ ] 4.3 ターン時間表示UI（通常モード）
  - **ファイル**: frontend/src/components/GameTimer.tsx（lines 373-384）
  - フォールバックモードと同じUI実装
  - useServerGameState.getCurrentTurnTime()を使用
  - data-testid="turn-time"属性を追加
  - **完了条件**: ブラウザで手動確認 + E2Eテストパス
```

### 2. フォールバックモードと通常モードの両方で実装

**このプロジェクトの2つのモード**:
- **フォールバックモード（Phase 1）**: useGameState、インメモリー状態管理
- **通常モード（Phase 2）**: useServerGameState、API連携とポーリング同期

**必須**: 新しいUI機能は**両方のモード**で実装する必要がある。
...
### ✅ バックエンド/ロジック層
- [ ] useGameStateに機能を実装（フォールバックモード用）
- [ ] useServerGameStateに機能を実装（通常モード用）
- [ ] ユニットテストで両方のhookを検証

### ✅ UI層（フォールバックモード）
- [ ] GameTimer.tsx（フォールバックモードセクション）にUI実装
- [ ] data-testid属性を追加
- [ ] ブラウザで手動確認

### ✅ UI層（通常モード）
- [ ] GameTimer.tsx（通常モードセクション）にUI実装
- [ ] フォールバックモードと同じUI構造を使用
- [ ] data-testid属性を追加（同じ値）
- [ ] ブラウザで手動確認

### ✅ E2Eテスト
- [ ] E2Eテストが通常モードで動作することを確認
- [ ] 全てのdata-testid要素が通常モードに存在することを確認
```
...
1. **コード検索で確認**:
   ```bash
   # data-testid属性がフォールバックモードと通常モードの両方に存在するか確認
   grep -n 'data-testid="turn-time"' frontend/src/components/GameTimer.tsx
   # 結果: 2行（フォールバックモード1行 + 通常モード1行）であることを確認
   ```

2. **ブラウザで確認**:
   - フォールバックモード: API接続失敗状態で動作確認
   - 通常モード: 正常なAPI接続状態で動作確認

3. **E2Eテストで確認**:
   - E2Eテストは通常モード（ブラウザ）で実行されるため、通常モードのUI実装が必須
   - テストが失敗する場合、通常モードにUIが実装されているか確認

### 4. 過去の失敗事例から学ぶ

**事例**: turn-time-tracking機能のE2Eテスト失敗

**問題**:
- ターン時間表示UIをフォールバックモードのみに実装
- 通常モードには実装しなかった
- E2Eテストが通常モード（ブラウザ）で実行されたため、10/15テストが失敗

**根本原因**:
- タスクにフォールバックモードと通常モードの分離が明記されていなかった
- 実装時にフォールバックモードのみで完了と判断してしまった

**再発防止策**:
- 全てのUI機能タスクを「バックエンド/ロジック層」「フォールバックモードUI」「通常モードUI」の3つに分割
- tasks.mdに明示的に記載
- E2Eテストで通常モードの実装を自動検証
